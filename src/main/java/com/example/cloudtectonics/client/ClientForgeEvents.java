 ...
 */
@Mod.EventBusSubscriber(modid = ModBlocks.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
public class ClientForgeEvents {
    ...
    public static void onRenderGuiPost(RenderGuiEvent.Post event) {
        if (!debugEnabled) return; // 关闭调试功能后，不显示圆形按钮
    ...
    public static void onKeyInput(InputEvent.Key event) {
        if (debugEnabled && DEBUG_KEY.consumeClick()) {
            toggleDebugLogging();
        }
    }
    ...
    public static void toggleDebugLogging() {
        ...
        if (BuildingDebugLogger.isLogging()) {
            ...
            if (path != null) {
                ...
                if (mc.player != null) {
                    mc.player.sendSystemMessage(Component.literal("停止日志记录失败！"));
                }
            }
        } else {
            BuildingDebugLogger.startLogging();
            if (mc.player != null) {
                mc.player.sendSystemMessage(Component.literal("调试日志记录已开始..."));
            }
        }
    }
    ...
    private static void renderProxyCollisionBoxes(RenderLevelStageEvent event, Minecraft mc) {
        if (!debugEnabled || !BuildingDebugLogger.isLogging()) return;
