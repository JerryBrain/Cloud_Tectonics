...
public class ServerboundUpdateBuildingPacket {
    ...
    public void handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            ServerLevel level = context.getSender().level().getServer().getLevel(context.getSender().level().dimension());
            if (level != null && level.isLoaded(pos)) {
                BlockEntity be = level.getBlockEntity(pos);
                if (be instanceof BuildingAnchorBlockEntity anchor) {
                    anchor.updateParameters(
                            bays, depths, widthMid, widthSide, depthStep, colHeight, roofPitch, eavesLen, gableSetback, cornerLift, dougongLv, preview,
