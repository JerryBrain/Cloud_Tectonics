...
public class StructureComponent {
    ...
    // 楔形斜面朝向 (0: 斜面向+Z, 1: 斜面向-Z, 2: 斜面向+X, 3: 斜面向-X)
    public final int wedgeDirection;

    public static float snapToPixel(float val) {
        return Math.round(val * 16.0f) / 16.0f;
    }
    ...
    public StructureComponent(Vector3f localPos, Vector3f size, Quaternionf rotation, String texture, float u0, float v0, float u1, float v1) {
        this(localPos, size, rotation, texture, u0, v0, u1, v1, 0xFFFFFFFF, ShapeType.CUBE, 0);
    }
    ...
    public StructureComponent(Vector3f localPos, Vector3f size, Quaternionf rotation, String texture, float u0, float v0, float u1, float v1, int color) {
        this(localPos, size, rotation, texture, u0, v0, u1, v1, color, ShapeType.CUBE, 0);
    }

    public StructureComponent(Vector3f localPos, Vector3f size, Quaternionf rotation, String texture, float u0, float v0, float u1, float v1, int color, ShapeType shape, int wedgeDirection) {
        float minX = localPos.x() - size.x() / 2.0f;
        float maxX = localPos.x() + size.x() / 2.0f;
        float minY = localPos.y() - size.y() / 2.0f;
        float maxY = localPos.y() + size.y() / 2.0f;
        float minZ = localPos.z() - size.z() / 2.0f;
        float maxZ = localPos.z() + size.z() / 2.0f;

        float sMinX = snapToPixel(minX);
        float sMaxX = snapToPixel(maxX);
        float sMinY = snapToPixel(minY);
        float sMaxY = snapToPixel(maxY);
        float sMinZ = snapToPixel(minZ);
        float sMaxZ = snapToPixel(maxZ);

        this.localPos = new Vector3f((sMinX + sMaxX) / 2.0f, (sMinY + sMaxY) / 2.0f, (sMinZ + sMaxZ) / 2.0f);
        this.size = new Vector3f(sMaxX - sMinX, sMaxY - sMinY, sMaxZ - sMinZ);
    ...
    public CompoundTag save(CompoundTag tag) {
        ...
        tag.putInt("wedgeDir", wedgeDirection);
        return tag;
    }
    ...
    public static StructureComponent load(CompoundTag tag) {
        ...
        ShapeType shape = tag.contains("shape") ? ShapeType.valueOf(tag.getString("shape")) : ShapeType.CUBE;
        int wedgeDir = tag.contains("wedgeDir") ? tag.getInt("wedgeDir") : 0;
        return new StructureComponent(
            pos, size, rot,
            tag.getString("texture"),
            tag.getFloat("u0"), tag.getFloat("v0"), tag.getFloat("u1"), tag.getFloat("v1"),
            color, shape, wedgeDir
        );
    }
